...
using Groupon.Migrations.SeedData;
...
namespace Groupon.Tests
{
    public abstract class GrouponTestBase : AbpIntegratedTestBase<GrouponTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected GrouponTestBase()
        {
        ...
        protected void UsingDbContext(Action<GrouponDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<GrouponDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<GrouponDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<GrouponDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<GrouponDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<GrouponDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<GrouponDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<GrouponDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<GrouponDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<GrouponDbContext, Task<T>> func)
        {
            T result;
